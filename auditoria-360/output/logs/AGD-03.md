# AGD-03 — CRIAR_DIA_SEMANA

**Funcionalidade:** agenda/01
**Data:** 2026-03-20T14:24:36Z
**Ambiente:** DEV (http://76.13.172.17:5678)

---

## Veredicto

**PASS**

### O que era esperado

O sistema deve interpretar 'segunda' como a proxima segunda-feira e criar evento as 10:30. O campo start_event deve ter horario com minutos correto.

### O que aconteceu

A mensagem "consulta no oftalmo segunda 10h30" foi enviada ao webhook dev. O sistema respondeu em log #3950.

O sistema se comportou conforme esperado. A operacao foi realizada corretamente e verificada no banco de dados real.

### Pontos positivos

Evento criado no banco. IA confirmou. Contagem subiu.

### Pontos negativos

Workflow N8N reportou error: 11688|CalendarWH|error
11685|CalendarWH|error

---

## KPIs do teste

| KPI | Valor | Descricao |
|-----|-------|-----------|
| Funcional | 1 | A funcionalidade fez o que deveria? (1=sim, 0=nao) |
| Banco | 1 | O banco de dados reflete a acao? (1=sim, 0=nao, 0.5=parcial) |
| N8N | 0 | Todos os workflows executaram sem error? (1=sim, 0=nao) |
| Consistencia | 1 | IA disse a mesma coisa que o banco mostra? (1=sim, 0=nao, 0.5=parcial) |
| **Score** | **0.75** | Media dos 4 KPIs (0 a 1) |

---

## Detalhes tecnicos

### Metodo utilizado

1. Snapshot ANTES: contou registros no banco (spent=91, calendar=318), salvou ultimo log_id=3949
2. Enviou mensagem "consulta no oftalmo segunda 10h30" via POST http://76.13.172.17:5678/webhook/dev-whatsapp
3. Polled log_users_messages a cada 3s ate novo log aparecer (max 45s)
4. Verificou banco imediatamente (operacao sincrona)
5. Snapshot DEPOIS: recontou registros (spent=91, calendar=319)
6. Buscou registro especifico por nome no banco
7. Consultou ultimas execucoes do N8N via API

### Resposta da IA

- **log_id:** 3950
- **Mensagem:**  

### Registro no banco

```
name=Consulta No Oftalmo|start=2026-03-23T10:30|end=2026-03-23T11:00|tipo=compromisso|rec=NAO|rrule=-|active=True|id=3ea3018b
```

### Execucoes N8N

```
11688|CalendarWH|error
11687|FixConflito|success
11686|Main|success
11685|CalendarWH|error
11684|FixConflito|success
11683|Main|success
```

### Snapshots

| Momento | spent | calendar | Variacao spent | Variacao calendar |
|---------|-------|----------|---------------|------------------|
| ANTES | 91 | 318 | — | — |
| DEPOIS | 91 | 319 | 0 | 1 |

---

*Teste executado por @auditor (Lupa) — Auditoria 360*
